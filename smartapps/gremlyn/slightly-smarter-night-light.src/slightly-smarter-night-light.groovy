...
preferences {
	section("Control these lights..."){
        ...
        input "level", "number", title: "How bright?, 1-100"
	}
...
def turnOffMotionAfterDelay() {
	log.trace "In turnOffMotionAfterDelay, state.motionStopTime = $state.motionStopTime, state.lastStatus = $state.lastStatus, lights.currentLevel = $lights.currentLevel, level = $level"
    if (state.motionStopTime && state.lastStatus != "off" && lights.currentLevel[0] == level) {
		def elapsed = now() - state.motionStopTime
